...
namespace HeroLib
{
    ...
    public static partial class RaycastExt
    {
        ...
        public static bool RayBetweenPoints(Vector3 p1, Vector3 p2, int collisionMask, float yOffset = 0.0f)
        {
            Vector3 vOffset = Vector3.up * yOffset;
            Vector3 rayDir = p2 - p1; rayDir.Normalize();
            return Physics.Raycast(p1 + vOffset, rayDir, Vector3.Distance(p1, p2), collisionMask);
        }

        public static bool RayBetweenPoints(Vector3 p1, Vector3 p2, int collisionMask, ref Vector3 hitPos)
        {
            Vector3 rayDir = p2 - p1; rayDir.Normalize();
            RaycastHit hitInfo;
            bool hit = Physics.Raycast(p1, rayDir, out hitInfo, Vector3.Distance(p1, p2), collisionMask);
            if (hit) hitPos = hitInfo.point;
            return hit;
        }

        public static bool SphereCastBetweenPoints(Vector3 p1, Vector3 p2, float radius, int collisionMask)
        {
            Vector3 toTarget = p2 - p1;
            float dist = toTarget.magnitude;
            toTarget.Normalize();
            RaycastHit hit;

            if (!Physics.SphereCast(p1, radius, toTarget, out hit, dist, collisionMask))
            {
                return false;
            }
            else return true;
        ...
        public static bool SphereCastBetweenPoints(Vector3 p1, Vector3 p2, float radius, int collisionMask, out RaycastHit rayHit)
        {
            Vector3 toTarget = p2 - p1;
            float dist = toTarget.magnitude;
            toTarget.Normalize();

            if (!Physics.SphereCast(p1, radius, toTarget, out rayHit, dist, collisionMask))
            {
                return false;
            }
            else return true;
