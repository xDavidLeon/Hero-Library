...
namespace HeroLib
{
	...
	public class ObjectPool
	{
		...
		public PooledObject Instantiate(Vector3 p, Quaternion q, Transform parent = null, float lifetime = -1)
		{
		...
		public static T Instantiate<T>(T prefab, Vector3 pos, Quaternion q, Transform parent = null, float lifetime = -1) where T : PooledObject
		{
			return (T) Get(prefab).Instantiate(pos, q, parent, lifetime);
		}
		...
		public static void Recycle(PooledObject po)
		{
			if (po != null)
			{
				...
				else
				{
					...
					if (_poolRoot == null)
					{
						_poolRoot = ObjectPoolRoot.Instance;
